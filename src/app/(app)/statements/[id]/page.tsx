...
export default function StatementDetailPage() {
  ...
  useEffect(() => {
    ...
    const base = getApiBase();
    void (async () => {
      const response = await authFetch(`${base}/credit-cards/${creditCardId}`);
      if (cancelled || !response.ok) return;
  ...
  async function confirmDeletePurchase() {
    ...
    try {
      const base = getApiBase();
      const response = await authFetch(`${base}/credit-card-purchases/${deletePurchaseId}`, { method: "DELETE" });
      if (!response.ok) {
        try {
  ...
  async function onCardChange(newCardId: string) {
    ...
    try {
      const base = getApiBase();
      const response = await authFetch(`${base}/credit-cards/${newCardId}/statements`);
      if (!response.ok) {
