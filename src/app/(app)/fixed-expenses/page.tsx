...
export default function FixedExpensesPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const base = getApiBase();
    const response = await authFetch(`${base}/fixed-expenses`, {
      method: "POST",
  ...
  async function disable(id: string) {
    const base = getApiBase();
    const response = await authFetch(`${base}/fixed-expenses/${id}/disable`, { method: "PATCH" });
    if (!response.ok) {
      toast.error("Erro");
      return;
    }
  ...
  async function changeFixedCategory(fixedId: string, newCategoryId: string) {
    ...
    try {
      const response = await authFetch(`${base}/fixed-expenses/${fixedId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function generate() {
    const base = getApiBase();
    const response = await authFetch(`${base}/fixed-expenses/generate-monthly-entries`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
