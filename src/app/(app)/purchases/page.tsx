...
function PurchasesPageInner() {
  ...
  useEffect(() => {
    ...
    void authFetch(`${base}/categories`).then(async (response) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const base = getApiBase();
    void authFetch(`${base}/credit-card-purchases?creditCardId=${encodeURIComponent(creditCardId)}`).then(
      async (response) => {
        if (cancelled || !response.ok) return;
  ...
  async function refreshPurchasesList(cardId: string) {
    if (!cardId) return;
    const base = getApiBase();
    const response = await authFetch(`${base}/credit-card-purchases?creditCardId=${encodeURIComponent(cardId)}`);
